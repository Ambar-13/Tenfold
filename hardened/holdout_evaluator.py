...
# ---------------------------------------------------------------------------
# HOLDOUT EVALUATOR. A copy of hardened/local_evaluator.py, written after
# the agent implementation was locked, with EVERY template bank
# (constraint paraphrase rules, opener frames, override frames, reply frames,
# no-preference / deflection / nudge frames) REWRITTEN in novel wording that
# shares no sentence frame with the hardened banks. Paraphrases are still
# derived ONLY from the original card string (true-of-target by construction);
# category anchor tokens stay verbatim in openers; scoring code is untouched.
# The RNG seed suffix is "holdout" so draws are independent of the hardened
# runs. Default tier is H1 (paraphrase-only), the tier the fix's H1 retention
...
HARDENED_TIER = os.environ.get("HARDENED_TIER", "H1").upper()
...
# HOLDOUT: template wordings are novel; no sentence frame is shared with the
# hardened banks in local_evaluator.py.
...
def paraphrase_constraint(text: str, rng: random.Random) -> str:
    ...
    if m:
        p = m.group(1).strip()
        return rng.choice([
            "price-wise, think ${p} give or take",
            "I'm hoping to land somewhere around ${p}",
            "${p}-ish is the zone I'm shopping in",
            "figure about ${p} as my spending target",
        ]).format(p=p)
    m = _COLOR_CARD_RE.match(t)
    if m:
        c = m.group(1).strip().lower()
        return rng.choice([
            "a {c} shade would suit me",
            "I'd go with {c} as the color",
            "the {c} version, please",
            "in {c}, ideally",
        ]).format(c=c)
    if low in MATERIALS:
        return rng.choice([
            "{m} would be my fabric of choice",
            "give me {m} if you can",
            "it ought to be {m}",
        ]).format(m=low)
    m = _PCT_RE.match(t)
    if m:
        ...
        if pct == "100":
            return rng.choice([
                ...
                "nothing but {r} in the fabric",
                "fully {r}, no blend",
            ]).format(r=rest)
        return rng.choice([
            "a mix that's {p} percent {r}",
            "{p} percent of it should be {r}",
        ]).format(p=pct, r=rest)
    if low == "machine wash":
        return rng.choice([
            "safe for the washing machine",
            "it should survive a machine wash",
            "launders in the machine without fuss",
        ])
    if low == "hand wash only":
        return rng.choice(["strictly hand wash territory", "washing by hand only, please"])
    if low == "imported":
        return rng.choice(["an import, that part's fine", "imported stock works for me"])
    m = _CLOSURE_RE.match(t)
    if m:
        c = m.group(1).strip().lower()
        if c in ("pull on", "pull-on"):
            return rng.choice([
                "the pull on kind, no buttons to fiddle with",
                "slips straight on to wear",
            ])
        return rng.choice([
            "fastens via {c}",
            "a {c} to do it up",
    ...
    if m:
        ...
        return rng.choice([
            "{v} when it comes to {k}",
            "as for {k}, make it {v}",
            "{k}-wise, {v}",
        ]).format(k=k, v=v)
    body = low
    return rng.choice([
        "it'd be great if it had {b}",
        "{b} would seal the deal",
        "put {b} on the list",
        "{b}, that's a thing I care about",
    ]).format(b=body)


_BUYING_OPENERS = (
    "Time to buy {cat}. The deal-breaker requirement: {c}.",
    "I've settled on getting {cat}; it absolutely has to have this: {c}.",
    "Purchasing {cat} soon, provided it meets one condition: {c}.",
    "On the hunt for {cat}. Hard requirement from my side: {c}.",
    "Ready to order {cat} once I find one with: {c}.",
)
_BROWSING_OPENERS = (
    "Window-shopping for {cat} right now.",
    "Curious what {cat} is out there; nothing settled yet.",
    "Give me a tour of the {cat} on offer; I'm undecided.",
    "Poking around for {cat}, keeping my options open.",
    "Early days - just scoping out {cat}.",
)
_OVERRIDE_OPENERS = (
    "Starting a search for {cat}. Current inclination: {c}.",
    "I'll be picking out {cat}. Tentatively I'd say: {c}.",
    "Browsing toward {cat}; leaning this way so far: {c}.",
    "{cat} shopping today. Initial thought: {c}.",
    "After some {cat}. If you pressed me now: {c}.",
)
_OVERRIDE_FRAMES = (
    "Scratch that last preference of mine ({old}); the thing I actually require: {new}.",
    "New plan. Set aside {old} entirely - it's {new} that I need.",
    "Having slept on it, {old} is out. In its place: {new}.",
    "Revision time: {new} is the requirement now, and {old} isn't.",
    "Please disregard what I said about {old}. The real must-have: {new}.",
    "Turns out {old} won't work for me after all. Focus on this instead: {new}.",
    "My priorities have shifted - {new} now tops the list, and {old} is off it.",
)
_REPLY_FRAMES = (
    "Since you ask: {c}.",
    "Sure - my answer there would be {c}.",
    "On that point, I'd want {c}.",
    "Happy to share: {c}. That's where I stand.",
    "If it helps narrow things down, {c}.",
    "Right, so: {c}. That's the shape of it.",
)
_NOPREF_FRAMES = (
    "There's nothing further I'd add on {a}.",
    "I'm drawing a blank on {a}, sorry.",
    "That about covers it as far as {a} goes.",
    "No additional {a} requirements from me.",
)
_DEFLECT_FRAMES = (
    "{a} isn't something I care much about - your call.",
    "Totally open on {a}; pick whatever seems right.",
    "I'd rather leave {a} up to you.",
    "Any {a} works for me, honestly - go with your gut.",
)
_NUDGE_FRAMES = (
    "Still missing the mark, I'm afraid. Quiz me on a particular attribute.",
    "These aren't landing for me. Pick one attribute and ask away.",
    "Closer, but no. Question me about a single specific attribute.",
)
_FILLER_HEADS = ("Okay, ", "Right, ", "To be fair, ", "Hm, ")
_FILLER_TAILS = (" or thereabouts.", " ...roughly speaking.", " That's the gist.")
...
class Perturber:
    ...
    def customer_reply(self, sample: dict, ask_attribute: object, disclosed: set[str], boundary_used: bool) -> tuple[str, bool]:
        ...
        joiner = rng.choice(["; ", ", and beyond that "]) if len(paras) > 1 else ""
...
def evaluate(
    ...
    for sample in samples:
        ...
        pert_rng = random.Random(
            f"{sample.get('sample_id', '')}\0{sample.get('scenario_type', '')}\0{HARDENED_TIER}\0holdout"
        )
